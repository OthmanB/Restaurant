...
def check_time_is_24h(time, y=None):
    '''
...
def check_time_is_1y(time, y=None):
    '''
...
def check_time_is_greater_than_period(time, period):
    ...
    if (np.max(time) - np.min(time)) < period:
        raise ValueError(
            "check_time_is_greater_than_period(): time interval smaller than requested period. "
            f"interval={np.max(time) - np.min(time)}, period={period}"
        )
    return [False]